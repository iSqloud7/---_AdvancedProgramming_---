package SecondColloquium.ColloquiumAssignments._2BookCollection.ComparatorWithLambdaExpressionAndAnonymousClass;
...
class Book { // implements Comparable<Book>
    ...
    /* not usefull for the second printing method
    @Override
    public int compareTo(Book other) {
        int result = this.getBookTitle().compareTo(other.getBookTitle());

        if (result == 0) { // this.bookTitle == other.bookTitle
            return Float.compare(this.getBookPrice(), other.getBookPrice());
        } else {
            return result;
        }
    }
    ...
    */
    ...
    public void printByCategory(String category) {
        books.stream()
                ...
                .sorted(TitleAndPriceComparator) // new TitleAndPriceComparator()
                .forEach(book -> System.out.println(book));
    }

    public List<Book> getCheapestN(int n) {
        return books.stream()
                .sorted(PriceComparator) // new PriceComparator()
                .limit(n)
